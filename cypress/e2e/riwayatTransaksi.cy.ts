describe("Riwayat Transaksi", () => {
  beforeEach(() => {
    cy.visit("/login");
    cy.get("input[id=email]").type("test12345@gmail.com");
    cy.get("input[id=password]").type("fuckyou");
  ...
  it("should filter transaksi", () => {
    ...
    cy.get("div#filterPopUp").find("input[id=startDate]").type("2021-01-01");
  ...
  it("pagination should work", () => {
    ...
    cy.get("div#riwayat-transaksi").find("div#pagination").should("be.visible");
    cy.get("div#riwayat-transaksi")
