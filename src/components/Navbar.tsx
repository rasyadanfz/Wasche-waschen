...
const Navbar = () => {
    ...
    const [currentPathName, setCurrentPathName] = useState("/login");
        ...
        const setData = async () => {
            ...
            if (session) {
                ...
                if (session.user.role === "Admin") {
                    setNavbarData(navbarDataAdmin);
                } else {
                    setNavbarData(navbarDataCustomer);
                }
            }
        };
            ...
            <Link id="cart" href={`/cart`}>
                <Image
                    ...
                    className={`transition-transform transform hover:scale-110 cursor-pointer ${
                        currentPathName === "/cart" ? "scale-110" : "scale-100"
                    }`}
                />
            </Link>
    ...
    return (
        <div>
            {disabledNavbar.includes(currentPathName) ? null : loading ? (
                <header
